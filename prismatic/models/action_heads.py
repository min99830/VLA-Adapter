...
class SimpleMLPActionHead(nn.Module):
    def __init__(
        self,
        input_dim=4096,
        hidden_dim=4096,
        ...
        self.task_k_proj = nn.Linear(hidden_dim, hidden_dim)  # Visual tokens with proprio
        self.task_v_proj = nn.Linear(hidden_dim, hidden_dim)  # Visual tokens with proprio
        self.action_query_proj = nn.Linear(hidden_dim, hidden_dim)

        self.mlp = nn.Sequential(
            nn.LayerNorm(input_dim * (ACTION_DIM + 1)),
            nn.Linear(input_dim * (ACTION_DIM + 1), hidden_dim),
            nn.ReLU(),
            nn.Linear(hidden_dim, action_dim),
        )

    def predict_action(self, actions_hidden_states, proprio=None, proprio_projector=None, phase="Inference"):
        B, num_layers, _, _ = actions_hidden_states.shape
        ...
        # Layer-wise max pooling
        pooled_actions_hidden_states = nn.functional.max_pool1d(
            actions_hidden_states.reshape(B, -1, num_layers),
            kernel_size=num_layers,
            stride=num_layers,
        ).reshape(B, -1, self.hidden_dim)

        task_hidden_states = pooled_actions_hidden_states[:, : self.num_task_tokens, :]
        hidden_states = pooled_actions_hidden_states[:, self.num_task_tokens :, :]

        conditions = torch.cat([task_hidden_states, proprio_features], dim=1)  # (B, cond_len, hidden_dim)
        k_task = self.task_k_proj(conditions)  # (B, cond_len, hidden_dim)
        v_task = self.task_v_proj(conditions)  # (B, cond_len, hidden_dim)
        a_query = self.action_query_proj(hidden_states)  # (B, action_dim * NUM_ACTIONS_CHUNK, hidden_dim)

        # Compute attention scores
        attn_scores = torch.matmul(a_query, k_task.transpose(-2, -1))  # (B, action_dim * NUM_ACTIONS_CHUNK, cond_len)
        attn_scores = attn_scores / math.sqrt(self.hidden_dim)
        attn_weights = torch.softmax(attn_scores, dim=-1)
        attn_output = torch.matmul(attn_weights, v_task)  # (B, action_dim * NUM_ACTIONS_CHUNK, hidden_dim)

        conditioned_actions_hidden_states = attn_output + hidden_states  # Residual connection

        action = self.mlp(conditioned_actions_hidden_states.reshape(B, NUM_ACTIONS_CHUNK, -1))
