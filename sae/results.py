...
project = "sparse_autoencoders"
# entity = "patrickaaleask" # removed
...
for sae_type in ["batchtopk", "topk"]:
    data = df[(df["config_sae_type"] == sae_type) & (df["k"] == 32.0)]
    data = data.sort_values(by="dictionary_size")
    axs[0, 0].plot(
        data["dictionary_size"], data["normalized_mse"], marker="o", linestyle="--", label=f"{sae_type} (k=32)"
    )
...
for sae_type in ["batchtopk", "topk", "jumprelu"]:
    ...
    data = data.sort_values(by="dictionary_size")
    axs[0, 1].plot(data["l0_norm"], data["normalized_mse"], marker="o", linestyle="--", label=sae_type)
...
for sae_type in ["batchtopk", "topk"]:
    ...
    axs[1, 0].plot(
        data["dictionary_size"], data["ce_degradation"], marker="o", linestyle="--", label=f"{sae_type} (k=32)"
    )
...
for sae_type in ["batchtopk", "topk", "jumprelu"]:
    data = df[(df["config_sae_type"] == sae_type) & (df["dictionary_size"] == 12288)]
    axs[1, 1].plot(data["l0_norm"], data["ce_degradation"], marker="o", linestyle="--", label=sae_type)
